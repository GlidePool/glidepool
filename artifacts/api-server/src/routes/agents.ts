...
router.post("/", async (req, res) => {
  ...
  const { userAddress, poolAddress, strategy, budgetUsdc, analysisIntervalSec, paymentTxHash } = parsed.data;
  ...
  try {
    ...
    logger.info({ agentId: agent!.id, userAddress, poolAddress }, "Agent created");
...
router.get("/:agentId", async (req, res) => {
  ...
  try {
    ...
    res.json({ ...agent, recentActions: actions });
  } catch (err) {

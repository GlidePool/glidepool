...
import {
  Bot, AlertCircle, Wallet2, Loader2,
...
const selectCls = "w-full bg-[#0d0d0d] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-white/80 focus:outline-none focus:border-primary/40 transition-all font-mono appearance-none cursor-pointer";
const inputCls  = "w-full bg-[#0d0d0d] border border-white/[0.08] rounded-lg px-3 py-2.5 text-sm text-white/80 focus:outline-none focus:border-primary/40 transition-all font-mono placeholder:text-white/20";
  ...
  if (!isConnected) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[55vh] gap-5 text-center px-4">
        <div className="w-14 h-14 rounded-2xl border border-primary/20 bg-primary/5 flex items-center justify-center">
  ...
  return (
    <div className="flex flex-col gap-5 max-w-xl mx-auto animate-in fade-in duration-400">
      ...
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl border border-primary/25 bg-primary/5 flex items-center justify-center">
          <Bot className="w-5 h-5 text-primary" />
        </div>
        <div>
          <h1 className="text-lg font-bold tracking-tight">AI Advisor</h1>
          <p className="text-[10px] text-white/35 font-mono mt-0.5">On-chain LP analysis · x402 micropayment gated</p>
        </div>
      </div>

      {/* Config */}
      <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-5 space-y-4">
        <div className="text-[10px] font-bold text-white/40 uppercase tracking-widest font-mono">Configure Analysis</div>
        ...
        <button
          ...
          className="w-full inline-flex items-center justify-center gap-2 px-5 py-3 rounded-lg font-semibold text-sm bg-primary text-[#080808] hover:opacity-90 transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {adviceLoading
      ...
      {/* Payment required */}
      {isPaymentRequired && (
        <div className="rounded-xl border border-amber-500/20 bg-amber-500/5 p-5 animate-in fade-in duration-300">
          <div className="flex gap-3 items-start">
            <CreditCard className="w-7 h-7 text-amber-400 shrink-0 mt-0.5" />
            <div>
          ...
          <div className="p-5 space-y-5">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {advice.recommendation.suggestedMode && (
                <span className="inline-flex items-center px-3 py-1.5 rounded border border-primary/20 text-xs font-mono text-primary/70">
            ...
            {(advice.recommendation.suggestedWithdrawPercent ?? 0) > 0 && (
              <div className="space-y-3">
                ...
                {txPreview && (
                  <div className="font-mono text-xs space-y-1.5 p-4 rounded-lg border border-white/[0.06] bg-black/30">
                    ...
                    <p className="text-[10px] text-white/20 mt-3 pt-3 border-t border-white/[0.05]">Sign in your wallet. Gas paid in ETH on Base.</p>
                  </div>
                )}
              </div>
            )}

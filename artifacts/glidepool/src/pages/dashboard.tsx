    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-400">
      ...
      <div className="flex items-start sm:items-center justify-between gap-3 flex-wrap">
        ...
        <Link href="/agent/setup">
          <button className="inline-flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-semibold bg-primary text-[#080808] hover:opacity-90 transition-opacity whitespace-nowrap">
        ...
        {[
          ...
          <div key={s.label} className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-4 flex flex-col gap-2">
            <div className="flex items-center justify-between">
              <span className="text-[9px] sm:text-[10px] text-white/30 uppercase tracking-widest font-mono leading-tight">{s.label}</span>
              <span className="text-white/20">{s.icon}</span>
            </div>
            <span className={`text-lg sm:text-xl font-bold font-mono truncate ${s.color}`}>{s.value}</span>
          </div>
        ))}
      </div>
      ...
      <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] overflow-hidden">
        <div className="flex items-center justify-between px-4 sm:px-5 py-3 border-b border-white/[0.06]">
          <span className="text-[10px] font-bold text-white/40 uppercase tracking-widest font-mono">Your Agents</span>
          <Link href="/monitor">
        ...
        {MOCK_AGENTS.length === 0 ? (
          ...
          <div className="divide-y divide-white/[0.04]">
            {MOCK_AGENTS.map((agent) => (
              <div key={agent.id} className="px-4 sm:px-5 py-3.5 flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 hover:bg-white/[0.02] transition-colors">
                {/* Left: icon + name */}
                ...
                {/* Right: status + pnl + last action */}
                <div className="flex items-center gap-3 flex-wrap pl-11 sm:pl-0">
                  <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded border text-[10px] font-mono font-bold ${STATUS_STYLES[agent.status]}`}>
        ...
        <div className="px-4 sm:px-5 py-2.5 border-t border-white/[0.05]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {[
          ...
          { href: "/pools",       icon: <Layers className="w-5 h-5 text-primary" />,   title: "Explore Pools", desc: "Browse Maverick V2 DLMM pools on Base Mainnet with live data." },
        ].map((card) => (
          <Link key={card.href} href={card.href}>
            <div className="rounded-xl border border-white/[0.07] bg-white/[0.02] p-4 sm:p-5 hover:border-primary/20 hover:bg-white/[0.04] transition-all cursor-pointer group h-full">
